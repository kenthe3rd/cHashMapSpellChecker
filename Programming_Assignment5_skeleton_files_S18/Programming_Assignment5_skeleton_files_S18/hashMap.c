...
void resizeTable(HashMap* map, int capacity)
{
    ...
    struct HashLink* copyCat;
    for(int i=0; i<map->capacity; ++i){
        copyCat = map->table[i];
        while(copyCat != NULL){
            hashMapPut(newMap, copyCat->key, copyCat->value);
...
void hashMapPut(HashMap* map, const char* key, int value)
{
    ...
    if(hashMapContainsKey(map, key)){
        int* keyedValue = hashMapGet(map, key);
        *keyedValue += value;
    } else {
...
void hashMapRemove(HashMap* map, const char* key)
{
    assert(map != 0);
    struct HashLink* searcher;
    struct HashLink* searcherPrev;
    int bucket;
    bucket = HASH_FUNCTION(key) % map->capacity;
    searcher = map->table[bucket];
    if(strcmp(searcher->key, key) == 0){
        map->table[bucket] = searcher->next;
